...
def define_parameters(user_config={}):
    ...
    inside the model code dir (i.e. `feffii/config/`).
    ...
    Remarks:
    1) Just the act of `import feffii` already sets up a default
    configuration.
    ...
    3) Config file feffii/config/default.yml should contain ALL config entries.
    ...
    Examples
    ----------
    1)  Use config file located in feffii/config/square.yml,
    ...
    config_file_path = os.path.join('feffii', 'config', 'default.yml')
...
def init_logging():
    """
    Initialize feffii logger. This happens after parameters have been defined.
    ...
    feffii_log_level = logging.DEBUG if config['very_verbose'] else logging.INFO

    flog.setLevel(feffii_log_level)
    ...
    fh = logging.FileHandler(
        ...
        mode='w',
        encoding='utf-8')
    fh.setLevel(feffii_log_level)
    fh.setFormatter(log_format)
    logging.getLogger('feffii').addHandler(fh)

    th = logging.StreamHandler()
    th.setLevel(feffii_log_level)
    th.setFormatter(log_format)
    logging.getLogger('feffii').addHandler(th)
