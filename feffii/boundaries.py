...
class Domain(object):
    """ Creates a simulation domain given its boundaries definitions and
        ...
        f_spaces : dict
                ...
                `feffii.functions.define_function_spaces()`.
        ...
        1)  Square with in/outflow on right side:
            ...
            mesh = feffii.mesh.create_mesh(domain='square')
            f_spaces = feffii.functions.define_function_spaces()
        ...
        2)  Custom geometry, with different subdomains+BCs on right side:
            ...
            domain = feffii.boundaries.Domain(
                ...
                f_spaces,
                boundaries = {
                  'top' : feffii.boundaries.Bound_Top(),
                  'top_right' : Bound_Top_Right(),
                  'bottom_right' : Bound_Bottom_Right(),
                  'bottom' : feffii.boundaries.Bound_Bottom(),
                  'left' : feffii.boundaries.Bound_Left()
                },
                BCs = {
                  'V' : {
                    'top' : [0, 'null'],
    ...
    def deform_boundary(self, boundary_label, goal_profile):
        ...
        Examples
        --------
        1)  feffii.parameters.define_parameters({
                'config_file' : 'feffii/config/lid-driven-cavity.yml',
            })
            ...
            simul = feffii.simulation.Simulation(f, domain.BCs)
