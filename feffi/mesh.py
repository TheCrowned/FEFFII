...
def create_mesh(**kwargs):
	...
	Relies on global config, but parameters can be passed overwriting the globals.
        ...
        Parameters
        ----------
        kwargs : `domain`, `mesh_resolution`, `domain_size_x`, `domain_size_y`, `shelf_size_x`, `shelf_size_y`, `mesh_resolution_x`, `mesh_resolution_y`, `mesh_resolution_sea_y` (refer to README for info).

        Examples
        --------
        1) Generate mesh on a 1x1 square domain, with mesh resolution 15x15:

           feffi.mesh(domain = 'square', mesh_resolution = 15)

        1) Generate mesh on a 3x2 domain, using globals for all other needed config:
