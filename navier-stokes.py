...
T = 10.0           # final time
num_steps = 10      # number of time steps per second
dt = T / num_steps # time step size
mu = 100             # kinematic viscosity
rho = 1            # density
...
def parse_commandline_args():
	...
	parser.add_argument('--mesh-resolution', default=16, dest='mesh_resolution', type=int)
	parser.add_argument('-v', default=False, dest='verbose', action='store_true')
	add_bool_arg(parser, 'plot', True)
	add_bool_arg(parser, 'plot_BC')
...
def define_function_spaces():
	global V, Q, u, v, p, q, u_n, u_, p_n, p_
...
def boundary_conditions():
	...
	bcu.append(DirichletBC(V.sub(1), Constant(0.0), bottom_boundary))
	bcu.append(DirichletBC(V.sub(1), Constant(0.0), above))
	bcu.append(DirichletBC(V.sub(0), Constant(0.0), left))
	bcu.append(DirichletBC(V.sub(1), Constant(0.0), left))
	
	bcp.append(DirichletBC(Q, Constant(0), above))
