...
steps_n = 1000
g = 0
nu_list = [1, 1e-1, 5e-2, 1e-2, 8e-3, 5e-3, 2e-3, 1e-3, 5e-4]
gammap_list = [0.25, 0.2, 0.15, 0.1, 0.075, 0.05]

csv_results = open(output_dir+'results.csv', 'w', newline='')
csv_fieldnames = ['nu', 'gammap', 'error_u', 'error_p']
csv_writer = csv.DictWriter(csv_results, fieldnames=csv_fieldnames)
csv_writer.writeheader()
#results_out = csv.writer(csv_results, delimiter=';', quotechar='|', quoting=csv.QUOTE_MINIMAL)

for nu in nu_list:

    for gammap in gammap_list:
        ...
        xdmffile = fenics.XDMFFile(output_dir+'nu-{}/gammap-{}/solutions.xdmf'.format(nu, gammap))
        ...
        solver_params = {'relative_tolerance': 1e-4, 'maximum_iterations': 10}
        ...
        xdmff = fenics.XDMFFile(output_dir+'nu-{}/gammap-{}/steady_solutions.xdmf'.format(nu, gammap))
        ...
        csv_writer.writerow({'nu': nu, 'gammap': gammap, 'error_u': err, 'error_p': err_p})
        csv_results.flush()
