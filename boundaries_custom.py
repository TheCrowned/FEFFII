...
class Bound_Ice_Shelf_Bottom(SubDomain):
	def inside(self, x, on_boundary):
		if((x[0] >= 0 and x[0] <= 0.4) and near(x[1], 0.9) and on_boundary):
			return True
...
class Bound_Ice_Shelf_Right(SubDomain):
	def inside(self, x, on_boundary):
		if(near(x[0], 0.4) and (x[1] >= 0.9 and x[1] <= 1) and on_boundary):
			return True
...
class Bound_Sea_Top(SubDomain):
	def inside(self, x, on_boundary):
		if(x[0] > 0.4 and near(x[1], 1) and on_boundary):
			return True
