...
# DO WE ACTUALLY NEED TO SET A CUSTOM TOLERANCE??
...
class Bound_Right(SubDomain):
	def inside(self, x, on_boundary):
		if(near(x[0], args.domain_size_x) and on_boundary):
			return True
...
class Bound_Ice_Shelf_Bottom(SubDomain):
	def inside(self, x, on_boundary):
		if((x[0] >= 0 and x[0] <= args.shelf_size_x) and near(x[1], args.domain_size_y - args.shelf_size_y) and on_boundary):
			return True
...
class Bound_Ice_Shelf_Right(SubDomain):
	def inside(self, x, on_boundary):
		if(near(x[0], args.shelf_size_x) and (x[1] >= args.domain_size_y - args.shelf_size_y and x[1] <= args.domain_size_y) and on_boundary):
			return True
